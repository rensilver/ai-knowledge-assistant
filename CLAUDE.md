...
- **API docs**: done — Swagger UI / OpenAPI 3 wired up via `springdoc-openapi-starter-webmvc-ui` (`OpenApiConfig`), with a JWT bearer security scheme and `@Operation`/`@ApiResponse` annotations documenting Auth, Documents, Chat, Agent, and Users.
...
- **`JwtFilter` reloads `UserDetails` from the database on every request** via `UserDetailsServiceImpl`, rather than trusting the role/authorities baked into the JWT's claims at issuance. That's why a role change made via `PATCH /users/{id}/role` takes effect on the very next request with no token refresh or re-login needed, and why this feature needs no token revocation/blacklist mechanism.
- **`./mvnw verify` can intermittently fail unrelated `AuthControllerIT` tests on low-memory dev machines**, once a second `*IT` class with a distinct `@TestPropertySource` exists (`AdminRoleManagementIT` is the first). Maven's default `reuseForks=true` keeps both classes' full Spring/Boot `ApplicationContext`s resident in the same Failsafe JVM fork for the rest of the run, and under memory pressure that manifests as Hikari/connection-timeout failures in tests that otherwise pass in isolation — proven environmental (reproduces identically with `AdminRoleManagementIT` removed; `./mvnw verify -DreuseForks=false` passes cleanly). If `verify` flakes locally with those symptoms, rerun with `./mvnw verify -DreuseForks=false`; changing `pom.xml`'s fork settings permanently is a build-policy call left to a human.
