...
public class RagService {
    ...
    public RagAnswer answer(String question, String conversationId) {
        ...
        String content = Timer.builder("rag.answer.duration")
                .register(meterRegistry)
                .record(() -> chatClient.prompt()
                        // Context goes in the system message, and the user message
                        // stays the raw question: MessageChatMemoryAdvisor persists
                        // only the last user message, so history records what was
                        // asked rather than a copy of every chunk retrieved for it.
